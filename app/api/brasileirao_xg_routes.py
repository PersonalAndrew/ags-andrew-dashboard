...
router = APIRouter(prefix="/api/brasileirao/xg", tags=["Brasileirao xG"])

PROJECT_ROOT = Path(__file__).resolve().parents[2]
DB_PATH = PROJECT_ROOT / "data" / "brasileirao_sofascore_xg.db"
...
def get_connection() -> sqlite3.Connection:
    if not DB_PATH.exists():
        raise HTTPException(
            status_code=404,
            detail="Base xG do Brasileirao nao encontrada. Rode: uv run python scripts/build_brasileirao_sofascore_xg.py",
        )
...
@router.get("/summary")
def brasileirao_xg_summary() -> dict[str, Any]:
    with get_connection() as con:
        total_shots = con.execute(
            "SELECT COUNT(*) AS total FROM brasileirao_sofascore_xg_shots"
        ).fetchone()["total"]

        total_matches = con.execute(
            "SELECT COUNT(*) AS total FROM brasileirao_sofascore_xg_matches"
        ).fetchone()["total"]

        total_teams = con.execute(
            "SELECT COUNT(*) AS total FROM brasileirao_sofascore_xg_teams"
        ).fetchone()["total"]

        total_players = con.execute(
            "SELECT COUNT(*) AS total FROM brasileirao_sofascore_xg_players"
        ).fetchone()["total"]

        leader_xg = con.execute(
            """
            SELECT team, xg_for, goals_for, xg_against, xg_difference
            FROM brasileirao_sofascore_xg_teams
        ...
        top_player = con.execute(
            ...
            FROM brasileirao_sofascore_xg_players
            ORDER BY xg DESC
            LIMIT 1
            """
        ).fetchone()
    ...
    return {
        ...
        "competition": "Brasileirao Betano 2026",
        ...
        "metrics": {
            ...
            "goals_minus_xg": "Gols marcados menos xG.",
    ...
    with get_connection() as con:
        rows = con.execute(
            ...
            SELECT
                ...
                away_xgot,
                total_xg
            FROM brasileirao_sofascore_xg_matches
            {where_sql}
            ORDER BY {order_by} {direction}, match_id ASC
            LIMIT ?
            """,
            (*params, limit),
        ).fetchall()
    ...
    with get_connection() as con:
        rows = con.execute(
            ...
            SELECT
                ...
                is_goal,
                is_on_target
            FROM brasileirao_sofascore_xg_shots
            {where_sql}
